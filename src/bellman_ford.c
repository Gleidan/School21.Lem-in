...
/*   Updated: 2020/11/11 10:34:20 by jconcent         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		add_to_queue(t_lem *lem, t_room *next_for_q, t_room *current_room, t_link *room_links)
{
	int		hash_key;
	t_room	*find_room;
	...
	if (room_links->weight == -1 && ft_strcmp(room_links->name, lem->start->name) != 0 && ft_strcmp(room_links->name, lem->end->name) != 0)
	...
	if (find_room->in_queue == 0 && room_links->on)
	{
		...
		if (!current_room->prev)
			...
			find_room->bellman_ford = find_room->prev->bellman_ford + room_links->weight;
