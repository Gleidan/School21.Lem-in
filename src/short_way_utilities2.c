...
/*   Updated: 2020/11/10 16:31:57 by jconcent         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void remove_crossed_links(t_lem *lem, t_room *find_room, t_link *find_link, t_path *tmp_path)
{
	int hash_key;
...
int	find_crossover(t_lem *lem, t_way *way)
{
	...
	while (tmp_path->forward)
	{
		...
		while (ft_strcmp(find_link->name, tmp_path->forward->name) != 0)
			find_link = find_link->next;
		if (find_link->in_way > 1 || (find_link->in_way >= 1 && check_reverse_link(lem, find_link->name, find_room->name)))
		{
			rt = 1;
